<script lang="ts">
  ...
  const terminalStates = new Set(['complete', 'partial', 'error', 'blocked', 'timeout', 'cached', 'manual']);
...
{#if retailers.length}<div class="retailer-strip">{#each retailers as retailer}<RetailerStatusItem {retailer} />{/each}</div>{/if}
