export type Offer = {
  retailer: string; seller: string | null; confidence: 'exact' | 'strong' | 'possible' | 'weak';
...
export type RetailerStatus = {
  retailer: string; state: 'pending' | 'running' | 'complete' | 'partial' | 'error' | 'blocked' | 'timeout' | 'cached' | 'manual';
...
export type SearchResult = {
  id: string; state: 'running' | 'complete'; offers: Offer[]; weak_matches: Offer[];
